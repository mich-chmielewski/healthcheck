...
public interface HitLogRepository extends JpaRepository<HitLog, Long> {
    ...
    @Query(value = "SELECT H.* FROM HIT_LOG H WHERE FORMATDATETIME(CREATED,'yyyy-MM-dd ') = CURRENT_DATE()"
            , nativeQuery = true)
    List<HitLog> findHitLogFromPresentDay();

    @Query(value = "SELECT H.* FROM HIT_LOG H WHERE FORMATDATETIME(CREATED,'yyyy-MM-dd ') = PARSEDATETIME(:fromday,'yyyy-MM-dd')"
            , nativeQuery = true)
    List<HitLog> findAllFromDay(@Param(value ="fromday") String fromDay);

    @Modifying
    @Transactional
    @Query(value = "DELETE FROM HIT_LOG WHERE FORMATDATETIME(CREATED,'yyyy-MM-dd ') = PARSEDATETIME(:date,'yyyy-MM-dd')"
            , nativeQuery = true)
